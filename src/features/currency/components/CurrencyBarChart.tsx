...
import type { Rates } from '@/features/currency/types';
 ...
 */
export default function CurrencyBarChart({ rates }: CurrencyBarChartProps) {
        ...
        <div className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {displayedEntries.map((item) => (
                    <div key={item.code} className="glass p-4 rounded-xl flex items-center gap-4 hover:bg-white/5 transition-colors">
                        <span className="w-12 font-bold text-white" translate="no">{item.code}</span>
                        ...
                        <span className="text-sm font-mono text-blue-300 min-w-[80px] text-right nums">
            ...
            {entries.length > 10 && (
                <div className="flex justify-center mt-8">
                    <button
                        type="button"
                        className="px-6 py-2.5 bg-white/5 border border-white/10 text-white rounded-xl hover:bg-white/10 hover:border-white/20 font-medium backdrop-blur-sm text-sm"
